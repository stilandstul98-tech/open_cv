...
# WITH ADVANCED TRACKING AND BACKGROUND FILTER
# ============================================
VIDEO_FILE = "bird4.mp4"

# ===== SETTINGS =====
MIN_AREA = 80                    # Minimum object area (noise filter)
MAX_AREA = 120  # облака больше этого
MIN_GROWTH_PERCENT = 10           # Minimum area growth % (camera shake filter)
GROW_FRAMES_THRESHOLD = 15     # How many consecutive frames object must grow
THRESHOLD_VALUE = 30             # Motion detection threshold
MAX_DISTANCE = 150               # Maximum distance for object tracking
...
print("=" * 70)
print("APPROACHING OBJECT DETECTION SYSTEM")
print("WITH ADVANCED TRACKING")
print("=" * 70)
print(f"Video: {VIDEO_FILE}")
print(f"Minimum growth: {MIN_GROWTH_PERCENT}%")
print(f"Frames for confirmation: {GROW_FRAMES_THRESHOLD}")
print(f"Maximum frames lost: {MAX_FRAMES_LOST}")
...
while cap.isOpened():
    ...
    # Find difference between frames (what changed)
    screen = cv2.absdiff(first_frame, gray)
    _, threshold = cv2.threshold(screen, THRESHOLD_VALUE, 255, cv2.THRESH_BINARY)
    ...
    for cont in conturs:
        ...
        # If area grew by MIN_GROWTH_PERCENT% or more - this is approaching!
        if growth_percent >= MIN_GROWTH_PERCENT and prev_area > 0:
            grow_count += 1
        else:
            # If growth less than 5% - this is noise or side movement
            grow_count = 0
        ...
        if is_locked:
            # RED square = locked target (grew 4+ frames once)
            color = (0, 0, 255)
        else:
        ...
        if is_locked:
            ...
            if grow_count >= GROW_FRAMES_THRESHOLD:
                print(f"[Frame {frame_number}] TARGET LOCKED! Area: {int(area)}, Growth: {growth_percent:.1f}%")
    ...
    cv2.putText(frame, f"Min Growth: {MIN_GROWTH_PERCENT}% | Threshold: {GROW_FRAMES_THRESHOLD} | Max Lost: {MAX_FRAMES_LOST}", 
                (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    # Show video
    cv2.imshow("Approaching Object Detection", frame)
